...
public class PinYin2Abbreviation {
	...
	private static int gbValue(char ch) {// 将一个汉字（GB2312）转换为十进制表示。
		...
		try {
			...
			byte[] s = new byte[] { (byte) 51456, (byte) 221 };
			System.out.println(new String(s));
			// System.out.println("A:" + (bytes[0] << 8 & 0xff00));
			// System.out.println("A:" + (bytes[1] & 0xff));
			int result = (bytes[0] << 8 & 0xff00) + (bytes[1] & 0xff);
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(cn2py("天空是蔚蓝的自由，你渴望着拥有，但愿那无拘无束的日子，将不再是一种奢求"));
	}
}
